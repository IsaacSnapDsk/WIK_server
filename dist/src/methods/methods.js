...
const calculateBets = (room) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (let i = 0; i < bets.length; i++) {
        ...
        if (win)
            round.winners.push(room.players[playerIdx]);
    }
...
const calculateScores = (room, io) => __awaiter(void 0, void 0, void 0, function* () {
...
module.exports = (io) => {
    ...
    const createRoom = function ({ roomName, maxRounds }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
    ...
    const joinRoom = function ({ joinId, nickname }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Find our room
                const room = yield roomModel.findOne({ 'joinId': joinId });
                //  If it isn't found, return an error
                if (!room)
                    return socket.emit("errorOccurred", "Please enter a valid join ID.");
                //  Check if our room has a player with this nickname already
                const existing = room.players.find((x) => x.name === nickname);
                ...
                const player = existing !== null && existing !== void 0 ? existing : new playerModel({
                    socketId: socket.id,
                    name: nickname,
                    connected: true,
                });
                ...
                //  Else, we should update the state of this existing player
                    ...
                    const actual = yield playerModel.findById(existing.id);
    ...
    const startGame = function ({ roomId, gmId }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                ...
                //  If it isn't found, return an error
                if (!room)
                    return socket.emit("errorOccurred", "Room not found.");
                //  Find our game master with this secret
                const gameMaster = yield gameMasterModel.findById(gmId);
                //  If it isn't found, return an error
                if (!gameMaster)
                    return socket.emit("errorOccurred", "Game Master not found");
                //  Check if this game master belongs to this room
                const belongs = gameMaster.roomId === roomId;
                //  If they dont, return an error
                if (!belongs)
                    return socket.emit("errorOccurred", "Game Master and Room do not match");
    ...
    const submitBet = function ({ roomId, bet }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
                ...
                const playerIdx = room.players.findIndex((x) => x._id.toString() === bet.playerId);
                console.log('player idx', playerIdx);
                //  Find the player matching this bet id
                const player = room.players[playerIdx];
                //  If it isn't found, return an error
                if (!player)
                    return socket.emit("errorOccurred", "Player does not exist.");
                //  Grab our current round
                const round = room.rounds[room.currentRound];
                //  Find the actual player model so we can update them
                const realPlayer = yield playerModel.findOne({ '_id': player.id });
     ...
     */
    const stopBetting = function ({ roomId, gmId }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
    ...
    */
    const stopWaiting = function ({ roomId, gmId, kill }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
                ...
                room.rounds[room.currentRound] = currentRound;
                //  Calculate
                const savedRoom = yield calculateBets(room);
                //  Return our new room
    ...
    const stopPunishing = function ({ roomId, gmId }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
                ...
                const savedRoom = yield calculateScores(room, io);
                //  Return our new room
                io.to(roomId).emit("changeTurnSuccess", savedRoom);
            }
            catch (e) {
    ...
    const submitScores = function ({ roomId, playerId, scores }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
                ...
                for (const score of scores) {
                    ...
                    //  Find the index of the vote for this player
                    const playerIdx = room.players.findIndex((x) => x._id.toString() === score.playerId);
                ...
                const socketPlayerIdx = room.players.findIndex((x) => x._id.toString() === playerId);
    ...
    const nextRound = function ({ roomId, gmId }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
    ...
    const startHalftime = function ({ roomId, gmId }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
    ...
    const stopHalftime = function ({ roomId, gmId }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
    ...
    const removePlayer = function ({ roomId, gmId, playerId }) {
        return __awaiter(this, void 0, void 0, function* () {
            const socket = this;
            try {
                //  Grab our current room
